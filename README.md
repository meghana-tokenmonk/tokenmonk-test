...
A required team plugin such as `tokenmonk-capture` is a different cache path and a different hook source. Do not treat its presence as proof that project hooks loaded, or as a substitute for this committed probe.
...
3. Optional: install the `meghud-tokenmonk-throwaway` plugin if you need the `probe.js` forwarder.
...
See [`docs/project-hooks.md`](docs/project-hooks.md) for the event list, stdout contract, cloud measurements, and common failures.
